...
public class ControladorAltaBaja {
	...
	private static String obtenerPilotos = "SELECT Manga.idManga FROM Piloto, Grupo, Manga WHERE Piloto.idGrupo"
			+ " = Grupo.idGrupo AND Grupo.idManga = Manga.idManga AND Manga.idPrueba = :idPrueba";
	
	private static String obtenerGrupo = "SELECT Grupo.idGrupo FROM Grupo, Manga WHERE Grupo.idManga = Manga.idManga AND Manga.idPrueba = :idPrueba";
	...
	public void setInicioApp(InicioApp app, Prueba prueba) {
		...
		empezar();
		@SuppressWarnings("unchecked")
		Query<Integer> qPil = session.createQuery(obtenerGrupo);
		qPil.setParameter("idPrueba", prueba.getIdPrueba());
		ArrayList<Integer> pilotos = (ArrayList<Integer>) qPil.list();
		terminar();
